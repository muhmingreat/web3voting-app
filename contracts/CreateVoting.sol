...
string memory _name,
    ...
    require(votingOrganiser == msg.sender,"Only organiser can  create the the voter");
...
function voterRight(address _address, string memory _name,
  string memory _image, string memory _ipfs ) public {
    require(votingOrganiser == msg.sender, "only Orgniser can create the voter");
    _voterId.increment();
    ...
    function vote(address _candidateAddress,uint256 _candidateVoteId) external {
        ...
        require(voter.voter_allowed != 0, "Yu have nowrite to vote");
